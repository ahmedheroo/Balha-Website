...
<app-navbar></app-navbar>
  ...
  <a href="tel:0567372301"
     class="group fixed bottom-6 right-6 z-50 flex items-center justify-center w-16 h-16 bg-[#52DE4B] text-white rounded-full shadow-2xl hover:bg-[#40b83a] hover:scale-110 transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-green-300 animate-in fade-in zoom-in duration-500"
     aria-label="اتصل بنا">

    <!-- Tooltip -->
    <span class="absolute right-full mr-3 px-3 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap shadow-xl pointer-events-none translate-x-2 group-hover:translate-x-0 hidden md:block">
      اتصل بنا
      <!-- Arrow pointing towards the button -->
      <span class="absolute top-1/2 left-full mt-1.5 border-[6px] border-transparent border-l-gray-900"></span>
    </span>
...
export class ReservationComponent {
  ...
  submitToWhatsApp() {
    ...
    // Open WhatsApp immediately (direct user action)
    ...
    const interval = setInterval(() => {
      ...
      if (this.countdown() <= 0) {
        ...
        // Hide modal and redirect to home
